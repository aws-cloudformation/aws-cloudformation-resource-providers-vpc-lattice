...
public class ListHandlerTest extends BaseTest {
    ...
    public void handleRequest_allApiCallsSuccess_successEvent() {
        ...
        mockSdkReturn(ListServicesResponse.builder()
    ...
    private List<ServiceSummary> getSummaries(final int numSummaries) {
        return IntStream
                .range(0, numSummaries)
                .mapToObj(
                        (i) -> ServiceSummary.builder()
                                .arn("service/svc-" + i)
                                .build()
                )
    ...
    private List<ResourceModel> toResourceModels(@Nonnull final List<ServiceSummary> summaries) {
        return summaries
                .stream()
