    ...
    protected ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        return ProgressEvent.progress(request.getDesiredResourceState(), callbackContext)
                .then((progress) -> proxy.initiate(
                                "AWS::VpcLattice::Service::ListServices",
                                proxyClient,
                                progress.getResourceModel(),
                                progress.getCallbackContext())
                        .translateToServiceRequest((model) -> Translator.createListServicesRequest(request.getNextToken()))
                        .makeServiceCall((listServicesRequest, client) ->
                                client.injectCredentialsAndInvokeV2(listServicesRequest, client.client()::listServices))
