    ...
    protected ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        return ProgressEvent.progress(request.getDesiredResourceState(), callbackContext)
                .then((progress) -> proxy.initiate(
                                "AWS::VpcLattice::Listener::ListListeners",
                                proxyClient,
                                progress.getResourceModel(),
                                progress.getCallbackContext())
                        .translateToServiceRequest((model) -> Translator.createListListenersRequest(model, request.getNextToken()))
                        .makeServiceCall((listListenersRequest, client) ->
                                client.injectCredentialsAndInvokeV2(listListenersRequest, client.client()::listListeners))
