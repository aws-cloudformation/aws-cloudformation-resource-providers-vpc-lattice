...
public class ListHandlerTest extends BaseTest {
    ...
    public void handleRequest_allApiCallsSuccess_successEvent() {
        ...
        mockSdkReturn(ListListenersResponse.builder()
    ...
    private List<ListenerSummary> getSummaries(final int numSummaries) {
        return IntStream
                .range(0, numSummaries)
                .mapToObj(
                        (i) -> ListenerSummary.builder()
                                .arn("service/svc-" + i)
                                .build()
                )
    ...
    private List<ResourceModel> toResourceModels(@Nonnull final List<ListenerSummary> summaries) {
        return summaries
                .stream()
