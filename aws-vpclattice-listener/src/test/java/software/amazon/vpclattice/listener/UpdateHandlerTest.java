...
public class UpdateHandlerTest extends BaseTest {
    ...
    public void handleRequest_allApiCallsSuccess_successEvent() {
        final var previousModel = getModel(MODEL_DEFAULT_RULE_ACTION);

        final var model = getModel(MODEL_UPDATED_RULE_ACTION);

        final var request = getRequest(previousModel, model).toBuilder()
                .desiredResourceTags(Map.of("foo", "bar"))
                .build();

        mockSdkReturn(UpdateListenerResponse.builder()
                .arn(LISTENER_ARN)
                .defaultAction(UPDATED_RULE_ACTION)
                .build());
        ...
        assertThat(response.getResourceModel().getArn()).isEqualTo(LISTENER_ARN);
    }

    @Test
    public void handleRequest_sameModel_successEvent() {
        final var request = getRequest(getModel(MODEL_DEFAULT_RULE_ACTION), getModel(MODEL_DEFAULT_RULE_ACTION));
    ...
    public void handleRequest_updateCallThrowsException_throwEquivalentHandlerErrorCode(
        ...
        final var request = getRequest(getModel(MODEL_DEFAULT_RULE_ACTION), getModel(MODEL_UPDATED_RULE_ACTION));
    ...
    private ResourceModel getModel(DefaultAction ruleAction) {
