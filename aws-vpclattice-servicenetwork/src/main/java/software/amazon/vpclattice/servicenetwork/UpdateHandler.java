...
public class UpdateHandler extends BaseHandlerStd {
    ...
    protected ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        return ProgressEvent.progress(request.getDesiredResourceState(), callbackContext)
                .then((progress) -> this.updateServiceNetwork(progress, proxy, request, proxyClient))
    ...
    private ProgressEvent<ResourceModel, CallbackContext> updateServiceNetwork(
            @Nonnull final ProgressEvent<ResourceModel, CallbackContext> progress,
            @Nonnull final AmazonWebServicesClientProxy proxy,
            @Nonnull final ResourceHandlerRequest<ResourceModel> request,
            @Nonnull final ProxyClient<software.amazon.awssdk.services.vpclattice.VpcLatticeClient> proxyClient) {
        if (!this.shouldUpdateServiceNetwork(request.getPreviousResourceState(), request.getDesiredResourceState())) {
            return progress;
        }

        return proxy.initiate(
                        "AWS::VpcLattice::ServiceNetwork::UpdateServiceNetwork",
                        proxyClient,
                        progress.getResourceModel(),
                        progress.getCallbackContext()
                )
                .translateToServiceRequest(Translator::createUpdateServiceNetworkRequest)
                .makeServiceCall((updateServiceNetworkRequest, client) ->
                        client.injectCredentialsAndInvokeV2(updateServiceNetworkRequest, client.client()::updateServiceNetwork))
    ...
    private ProgressEvent<ResourceModel, CallbackContext> updateTags(
        ...
        return proxy.initiate("AWS::VpcLattice::ServiceNetwork::UpdateTags", proxyClient, progress.getResourceModel(), progress.getCallbackContext())
    ...
    private boolean shouldUpdateServiceNetwork(
