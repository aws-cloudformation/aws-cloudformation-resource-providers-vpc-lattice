...
public class ReadHandler extends BaseHandlerStd {
    ...
    protected ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        return ProgressEvent.progress(request.getDesiredResourceState(), callbackContext)
                .then((progress) -> proxy.initiate(
                                "AWS::VpcLattice::ServiceNetwork::GetServiceNetwork",
                                proxyClient,
                                progress.getResourceModel(),
                                progress.getCallbackContext())
                        .translateToServiceRequest(Function.identity())
                        .makeServiceCall((model, client) -> {
                            final var getServiceNetworkResponse = getServiceNetwork(client, model);

                            final var listTagsResponse = TagHelper.listTags(model.getArn(), client);

                            return Translator.createResourceModel(getServiceNetworkResponse, listTagsResponse);
                        })
                        .handleError(ExceptionHandler::handleError)
                        .done(ProgressEvent::defaultSuccessHandler));
    }

    private GetServiceNetworkResponse getServiceNetwork(
        ...
        return client.injectCredentialsAndInvokeV2(Translator.createGetServiceNetworkRequest(model), client.client()::getServiceNetwork);
    }
    ...
    private String getArnByName(
        ...
        do {
            final var listRequest = Translator.createListServiceNetworksRequest(nextToken);

            final var listResponse =
                    client.injectCredentialsAndInvokeV2(listRequest, client.client()::listServiceNetworks);

            final var items = Optional.ofNullable(listResponse.items())
                    .orElse(List.of());

            for (final var serviceNetwork : items) {
                if (serviceNetwork.name().equals(name)) {
                    return serviceNetwork.arn();
                }
            }
