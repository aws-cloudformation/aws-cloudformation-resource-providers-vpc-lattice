...
public class CreateHandler extends BaseHandlerStd {
    ...
    protected ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        return ProgressEvent.progress(request.getDesiredResourceState(), callbackContext)
                .then((progress) -> this.createServiceNetwork(progress, proxy, request, proxyClient))
    ...
    protected ProgressEvent<ResourceModel, CallbackContext> createServiceNetwork(
            @Nonnull final ProgressEvent<ResourceModel, CallbackContext> progress,
            @Nonnull final AmazonWebServicesClientProxy proxy,
            @Nonnull final ResourceHandlerRequest<ResourceModel> request,
            @Nonnull final ProxyClient<VpcLatticeClient> proxyClient) {
        return proxy.initiate(
                        "AWS::VpcLattice::ServiceNetwork::CreateServiceNetwork",
                        proxyClient,
                        progress.getResourceModel(),
                        progress.getCallbackContext()
                )
                .translateToServiceRequest((_model) -> Translator.createCreateServiceNetworkRequest(request))
                .makeServiceCall(((createServiceNetworkRequest, client) ->
                        client.injectCredentialsAndInvokeV2(createServiceNetworkRequest, client.client()::createServiceNetwork)))
                .handleError(ExceptionHandler::handleError)
                .done((createServiceNetworkResponse) -> {
                    final var model = progress.getResourceModel();

                    model.setArn(createServiceNetworkResponse.arn());
