    ...
    protected ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        return ProgressEvent.progress(request.getDesiredResourceState(), callbackContext)
                .then((progress) -> proxy.initiate(
                                "AWS::VpcLattice::ServiceNetwork::ListServiceNetworks",
                                proxyClient,
                                progress.getResourceModel(),
                                progress.getCallbackContext())
                        .translateToServiceRequest((model) -> Translator.createListServiceNetworksRequest(request.getNextToken()))
                        .makeServiceCall((listServiceNetworksRequest, client) ->
                                client.injectCredentialsAndInvokeV2(listServiceNetworksRequest, client.client()::listServiceNetworks))
