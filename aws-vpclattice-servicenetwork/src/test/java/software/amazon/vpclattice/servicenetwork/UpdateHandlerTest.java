...
public class UpdateHandlerTest extends BaseTest {
    ...
    public void handleRequest_allApiCallsSuccess_successEvent() {
        final var previousModel = getModel(AuthType.NONE);

        final var model = getModel(AuthType.AWS_IAM);

        final var request = getRequest(previousModel, model).toBuilder()
                .desiredResourceTags(Map.of("foo", "bar"))
                .build();

        mockSdkReturn(UpdateServiceNetworkResponse.builder()
                .arn(SERVICE_NETWORK_ARN)
                .authType(AuthType.AWS_IAM)
                .build());

        mockSdkReturn(getServiceNetwork());
        ...
        assertThat(response.getResourceModel().getArn()).isEqualTo(SERVICE_NETWORK_ARN);
    }

    @Test
    public void handleRequest_sameModel_successEvent() {
        final var request = getRequest(getModel(software.amazon.awssdk.services.vpclattice.model.AuthType.NONE), getModel(software.amazon.awssdk.services.vpclattice.model.AuthType.NONE));
    ...
    public void handleRequest_updateCallThrowsException_throwEquivalentHandlerErrorCode(
        ...
        final var request = getRequest(getModel(software.amazon.awssdk.services.vpclattice.model.AuthType.NONE), getModel(software.amazon.awssdk.services.vpclattice.model.AuthType.AWS_IAM));
    ...
    private ResourceModel getModel(AuthType authType) {
