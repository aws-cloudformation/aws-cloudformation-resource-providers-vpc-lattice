...
public class ListHandlerTest extends BaseTest {
    ...
    public void handleRequest_allApiCallsSuccess_successEvent() {
        ...
        mockSdkReturn(ListServiceNetworkServiceAssociationsResponse.builder()
    ...
    private ResourceHandlerRequest<ResourceModel> getRequestWithNextToken() {
        return ResourceHandlerRequest.<ResourceModel>builder()
                .awsAccountId(AWS_ACCOUNT_ID)
                .region(REGION)
                .desiredResourceState(getModel())
    ...
    private List<ServiceNetworkServiceAssociationSummary> getSummaries(final int numSummaries) {
        return IntStream
                .range(0, numSummaries)
                .mapToObj(
                        (i) -> ServiceNetworkServiceAssociationSummary.builder()
                                .id("snva-id" + i)
                                .arn("snva/mva-" + i)
                                .status(ServiceNetworkServiceAssociationStatus.ACTIVE)
    ...
    private List<ResourceModel> toResourceModels(@Nonnull final List<ServiceNetworkServiceAssociationSummary> summaries) {
        return summaries
                .stream()
