    ...
    protected ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        return ProgressEvent.progress(request.getDesiredResourceState(), callbackContext)
                .then((progress) -> proxy.initiate(
                                "AWS::VpcLattice::Rule::ListRules",
                                proxyClient,
                                progress.getResourceModel(),
                                progress.getCallbackContext())
                        .translateToServiceRequest((model) -> Translator.createListRulesRequest(model, request.getNextToken()))
                        .makeServiceCall((listRulesRequest, client) ->
                                client.injectCredentialsAndInvokeV2(listRulesRequest, client.client()::listRules))
