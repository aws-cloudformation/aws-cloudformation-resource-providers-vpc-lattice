...
public class ListHandlerTest extends BaseTest {
    ...
    public void handleRequest_allApiCallsSuccess_successEvent() {
        ...
        mockSdkReturn(ListRulesResponse.builder()
    ...
    private List<RuleSummary> getSummaries(final int numSummaries) {
        return IntStream
                .range(0, numSummaries)
                .mapToObj(
                        (i) -> RuleSummary.builder()
                                .arn("service/svc-" + i)
                                .build()
                )
    ...
    private List<ResourceModel> toResourceModels(@Nonnull final List<RuleSummary> summaries) {
        return summaries
                .stream()
