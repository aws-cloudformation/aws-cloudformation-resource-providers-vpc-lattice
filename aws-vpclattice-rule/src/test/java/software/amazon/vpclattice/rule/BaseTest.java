...
public abstract class BaseTest {
    ...
    public static final String RULE_ARN = String.format(
            "arn:aws:vpc-lattice:%s:%s:service/%s/listener/%s/rule/%s",
            REGION,
            AWS_ACCOUNT_ID,
            SERVICE_ID,
            LISTENER_ID,
    ...
    public static final Integer RULE_PRIORITY = 50;

    public static final String NEXT_TOKEN = "DUMMY_TOKEN";

    public static final software.amazon.awssdk.services.vpclattice.model.RuleMatch RULE_MATCH =
            software.amazon.awssdk.services.vpclattice.model.RuleMatch.builder().build();

    public static final software.amazon.awssdk.services.vpclattice.model.RuleAction RULE_ACTION =
            software.amazon.awssdk.services.vpclattice.model.RuleAction.builder().build();

    public static final Match MODEL_RULE_MATCH = Match.builder()
            .httpMatch(HttpMatch.builder().method("foo").build())
            .build();

    public static final Match UPDATED_MODEL_RULE_MATCH = Match.builder()
            .httpMatch(HttpMatch.builder().method("bar").build())
            .build();

    public static final Action MODEL_RULE_ACTION = Action.builder().build();

    protected GetRuleResponse getRule(final int priortiy) {
    ...
    protected void mockSdkReturn(@Nonnull final UpdateRuleResponse response) {
        doReturn(response).when(proxyClient.client()).updateRule(any(UpdateRuleRequest.class));
    }

    protected void mockSdkReturn(@Nonnull final GetRuleResponse response, @Nonnull final GetRuleResponse... responses) {
        ...
        stubber.when(proxyClient.client()).getRule(any(GetRuleRequest.class));
    }

    protected void mockSdkReturn(@Nonnull final CreateRuleResponse response) {
        doReturn(response).when(proxyClient.client()).createRule(any(CreateRuleRequest.class));
    }

    protected void mockSdkReturn(@Nonnull final DeleteRuleResponse response) {
        doReturn(response).when(proxyClient.client()).deleteRule(any(DeleteRuleRequest.class));
    }

    protected void mockSdkReturn(@Nonnull final ListRulesResponse response) {
        doReturn(response).when(proxyClient.client()).listRules(any(ListRulesRequest.class));
    }

    protected void mockSdkGetThrow(@Nonnull final Class<? extends Throwable> exception) {
        doThrow(exception).when(proxyClient.client()).getRule(any(GetRuleRequest.class));
    }

    protected void mockSdkCreateThrow(@Nonnull final Class<? extends Throwable> exception) {
        doThrow(exception).when(proxyClient.client()).createRule(any(CreateRuleRequest.class));
    }
    ...
    protected void mockSdkUpdateThrow(@Nonnull final Class<? extends Throwable> exception) {
        doThrow(exception).when(proxyClient.client()).updateRule(any(UpdateRuleRequest.class));
    }

    protected void mockSdkDeleteThrow(@Nonnull final Class<? extends Throwable> exception) {
        doThrow(exception).when(proxyClient.client()).deleteRule(any(DeleteRuleRequest.class));
    }

    protected void mockSdkListThrow(@Nonnull final Class<? extends Throwable> exception) {
        doThrow(exception).when(proxyClient.client()).listRules(any(ListRulesRequest.class));
    }
}
